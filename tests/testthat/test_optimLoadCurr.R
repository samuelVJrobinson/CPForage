...
test_that('Currency calculations work properly',{
  test1<-optimLoadCurr(u,nests,world)
  ...
  nests[[1]]$whatCurr='rat' #Rate maximizers
  test1b<-optimLoadCurr(u,nests,world)
  ...
  #Rate vs Rate
  nests$nest2=nests$nest1 #Add 2nd nest
  test1c<-optimLoadCurr(u,nests,world)
  ...
  #Rate vs Eff
  nests$nest2$whatCurr='eff' #Changes 2nd nest to Eff
  test1d<-optimLoadCurr(u,nests,world)
  ...
  nests$nest1$sol=T #Changes both nests to solitaries
  nests$nest2$sol=T
  test1e<-optimLoadCurr(u,nests,world)
...
test_that('Exception handling works properly',{
  ...
  nests[[1]]$n=0
  test2<-optimLoadCurr(u,nests,world)
  ...
  #Worthless patch
  world$mu=0
  test3<-optimLoadCurr(u,nests,world)
  expect_identical(unname(test3$optimL),NA) #L should be NA
  expect_identical(unname(test3$optimCurr),-1) #curr should be -Inf (or -Inf for rate maximizers)
  expect_identical(unname(test3$S),NA) #S should be NA
  world$mu=8.33e-05

  #Forager arguments NA
  nests[[1]]$whatCurr=NA
  expect_error(optimLoadCurr(u,nests,world))

  #Forager arguments missing
  nests[[1]]$whatCurr=NULL
  expect_error(optimLoadCurr(u,nests,world))
